...
PDNode *patterns::Reshape2Mul::operator()() {
  auto reshape2_in_x = pattern->NewNode(reshape2_in_x_repr())
                           ->assert_is_op_input("reshape2", "X")
                           ->AsInput();
  auto reshape2_op =
      pattern->NewNode(reshape2_op_repr())->assert_is_op("reshape2");
  auto mul_in_x = pattern->NewNode(mul_in_x_repr())
                         ->assert_is_op_output("reshape2", "Out")
                         ->assert_is_op_input("mul", "X");
  auto mul_in_y =
      pattern->NewNode(mul_in_y_repr())->assert_is_op_input("mul", "Y");
  auto mul_op = pattern->NewNode(mul_op_repr())->assert_is_op("mul");
  auto mul_out = pattern->NewNode(mul_out_repr())
                        ->AsOutput()
                        ->assert_is_op_output("mul", "Out");

  reshape2_op->LinksFrom({reshape2_in_x}).LinksTo({mul_in_x});
  mul_op->LinksFrom({mul_in_x, mul_in_y}).LinksTo({mul_out});
  return mul_out;
}
