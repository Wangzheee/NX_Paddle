...
// Reshape2 + Mul
// Forward pass.
struct Reshape2Mul : public PatternBase {
  Reshape2Mul(PDPattern* pattern, const std::string& name_scope)
      : PatternBase(pattern, name_scope, "reshape2_mul") {}

  PDNode* operator()();
  PATTERN_DECL_NODE(reshape2_in_x);
  PATTERN_DECL_NODE(reshape2_op);
  PATTERN_DECL_NODE(mul_in_x);
  PATTERN_DECL_NODE(mul_in_y);
  PATTERN_DECL_NODE(mul_op);
