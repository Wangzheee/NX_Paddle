...
void MapMatmul2MulPass::ApplyImpl(ir::Graph* graph) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    flag = flag && std::abs(alpha - 1.0) < 1e-5;
...
void Reshape2MatmulFusePass::ApplyImpl(ir::Graph* graph) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    flag = flag && matmul_in_x_rank == 2 && matmul_in_y_rank == 2;

    std::vector<Node*>& next_ops = matmul_out->outputs;
    flag = flag && next_ops.size() == 1 &&
           next_ops[0]->Name() == "elementwise_add";
    std::cout << "reshape2 + matmul flag: " << flag << "\n";
    if (flag) {
      OpDesc desc;
      if (transpose_Y) {
        auto* scope = param_scope();
        auto* y_tensor =
            scope->FindVar(matmul_in_y->Name())->GetMutable<LoDTensor>();
        auto y_dims = y_tensor->dims();
        std::vector<int64_t> ys = {y_dims[1], y_dims[0]};
        framework::DDim* new_y_dims = new framework::DDim(ys.data(), 2);
        y_tensor->Resize(*new_y_dims);
        desc.SetAttr("transpose_Y", true);
      }
      desc.SetType("mul");
      ...
      desc.SetAttr("reshape_fusion", true);
      if (matmul_op->Op()->HasAttr("enable_int8")) {
        desc.SetAttr("enable_int8", matmul_op->Op()->GetAttr("enable_int8"));
        desc.SetAttr("X_scale", matmul_op->Op()->GetAttr("X_scale"));
        desc.SetAttr("weight_scale", matmul_op->Op()->GetAttr("weight_scale"));
...
void Reshape2MulFusePass::ApplyImpl(ir::Graph* graph) const {
  PADDLE_ENFORCE_NOT_NULL(
      graph, platform::errors::InvalidArgument("Graph cannot be nullptr."));
  std::string name_scope = "reshape2_mul_fuse_pass";
  FusePassBase::Init(name_scope, graph);

  GraphPatternDetector gpd;
  patterns::Reshape2Mul fuse_pattern(gpd.mutable_pattern(), name_scope);
  fuse_pattern();

  int found_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* g) {
    VLOG(1) << "fuse reshape2+mul to mul";
    GET_IR_NODE_FROM_SUBGRAPH(reshape2_in_x, reshape2_in_x, fuse_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(reshape2_op, reshape2_op, fuse_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(mul_in_x, mul_in_x, fuse_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(mul_in_y, mul_in_y, fuse_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(mul_op, mul_op, fuse_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(mul_out, mul_out, fuse_pattern);
    bool flag = true;
    ...
    std::vector<int> reshape2_op_shape =
        BOOST_GET_CONST(std::vector<int>, reshape2_op->Op()->GetAttr("shape"));
    flag = flag && reshape2_in_nums == 1 && reshape2_in_x_rank == 4 &&
           reshape2_op_shape.size() == 2 && (mul_in_x->outputs).size() == 1;

    size_t mul_in_x_rank = (mul_in_x->Var()->GetShape()).size();
    size_t mul_in_y_rank = (mul_in_y->Var()->GetShape()).size();
    flag = flag && mul_in_x_rank == 2 && mul_in_y_rank == 2;

    std::vector<Node*>& next_ops = mul_out->outputs;
    flag = flag && next_ops.size() == 1 &&
           next_ops[0]->Name() == "elementwise_add";
    std::cout << "reshape2 + mul flag: " << flag << "\n";
    if (flag) {
      OpDesc desc;
      desc.SetType("mul");
      desc.SetInput("X", {reshape2_in_x->Name()});
      desc.SetInput("Y", {mul_in_y->Name()});
      desc.SetOutput("Out", {mul_out->Name()});
      desc.SetAttr("x_num_col_dims", 1);
      desc.SetAttr("y_num_col_dims", 1);
      desc.SetAttr("reshape_fusion", true);
      if (mul_op->Op()->HasAttr("enable_int8")) {
        desc.SetAttr("enable_int8", mul_op->Op()->GetAttr("enable_int8"));
        desc.SetAttr("X_scale", mul_op->Op()->GetAttr("X_scale"));
        desc.SetAttr("weight_scale", mul_op->Op()->GetAttr("weight_scale"));
      }
      auto mul_node = g->CreateOpNode(&desc);
      IR_NODE_LINK_TO(reshape2_in_x, mul_node);
      IR_NODE_LINK_TO(mul_in_y, mul_node);
      IR_NODE_LINK_TO(mul_node, mul_out);
      GraphSafeRemoveNodes(graph, {reshape2_op, mul_in_x, mul_op});
      ++found_count;
    }
  };
...
REGISTER_PASS(reshape2_mul_fuse_pass,
              paddle::framework::ir::Reshape2MulFusePass);
REGISTER_PASS_CAPABILITY(reshape2_mul_fuse_pass)
    .AddCombination(
        paddle::framework::compatible::OpVersionComparatorCombination()
            .EQ("reshape2", 0)
            .EQ("mul", 0));
